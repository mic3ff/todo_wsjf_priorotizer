...
def createTask():
    ...
    global urgency
    urgency = (input("urgent? (y/n): "))
    global importance
    importance = (input("important? (y/n): "))
    print()
    ...
    calculateStrategy(urgency, importance)

    task.extend([summary, value, size, timeCriticality, riskReduction, cost_of_delay, wsjf, urgency, importance, strategy])
...
def sortTasks(list): # sort by WSJF
    for task in tasks:
        task[6] = float(task[6])
...
def option1():
    ...
    ## Tables, take 1
    print(tabulate(tasks, headers=["TASK", "VALUE", "SIZE", "CRIT.", "RISK RED.", "CoD", "WSJF", "IMP.", "URG.", "STRATEGY"], tablefmt='grid'))

    # display onlyu summary, wsjf and strategy:
    '''
    new_tasks = list()
    for task in tasks:
        new_task = list()
        #new_task = (task[0], task[6], task[9])
        new_task.append(task[0])
        new_task.append(task[6])
        new_task.append(task[7])

        new_tasks.append(new_task)

    print(tabulate(new_tasks, headers=["TASK", "WSJF", "TEST"], tablefmt='grid'))
    '''
